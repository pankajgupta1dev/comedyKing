...
const ffmpeg = process.platform === "win32" ? path.join(__dirname, "bin", "ffmpeg.exe") : "ffmpeg";
...
async function cutVideo() {
  ...
  if (start >= db.total_duration) {
    ...
    if (await fs.pathExists(videoPath)) {
      ...
      console.log("🗑️ Downloaded video removed.");
    }
...
"${outputPath}"`;
    ...
    const filter = "[0:v]scale=1080:1920:force_original_aspect_ratio=increase,boxblur=25:5,crop=1080:1920[bg];" + "[0:v]scale=1080:1920:force_original_aspect_ratio=decrease[fg];" + "[bg][fg]overlay=(W-w)/2:(H-h)/2";
